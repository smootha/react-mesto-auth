...
function Header({ loggedIn, onLogout, userEmail, isMenuOpen, setMenuOpen }) {
  ...
  return (
    // Добавляет класс который прячет меню с почтой и кнопкой логаута ща пределы страницы над шапкой
        ...
        {loggedIn && (
          <div
            className={`button header__burger-menu ${
              isMenuOpen ? "header__burger-menu_opened" : ""
            }`}
            onClick={setMenuOpen}
          >
          </div>
          ...
          element={
            <div
              className={`header__button-container ${
                isMenuOpen ? "header__button-container_opened" : ""
              }`}
            >
